...
export const About = () => {
    return (
        <section className="py-14 md:py-[100px] w-full bg-gradient-to-b from-background-primary via-background-secondary to-background-primary overflow-hidden border-y border-black/[0.04]">
            <div className="max-w-7xl mx-auto px-5">
                <SectionReveal className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-20 items-center">
                    ...
                    <div className="flex flex-col">
                        <motion.div variants={itemVariants} className="mb-4 md:mb-6">
                            <span className="text-[10px] md:text-xs font-semibold uppercase tracking-[0.1em] text-accent border border-accent/15 bg-accent/[0.06] rounded-full px-3 md:px-4 py-1 md:py-1.5 backdrop-blur-sm inline-block">
                                About Your Trainer
                            </span>
                        </motion.div>
                        ...
                        <motion.h2
                            variants={itemVariants}
                            className="text-[26px] md:text-[40px] font-space-grotesk font-bold text-text-primary mb-4 md:mb-6 leading-tight"
                        >
                        ...
                        <motion.p
                            variants={itemVariants}
                            className="text-[15px] md:text-lg text-text-secondary leading-relaxed mb-6 md:mb-10"
                        >
                        ...
                        <motion.div variants={itemVariants} className="w-full overflow-x-auto no-scrollbar pb-4 md:pb-6 -mx-5 px-5 lg:mx-0 lg:px-0">
                            <div className="flex gap-2 md:gap-3 w-max">
                                {["Level 4 PT", "Sports Rehab", "Nutrition Specialist", "Master Trainer"].map((badge, idx) => (
                                    <div key={idx} className="bg-white border border-accent/10 rounded-full px-3 md:px-4 py-1.5 md:py-2 text-[12px] md:text-[13px] font-medium text-text-primary whitespace-nowrap shadow-card">
                                        {badge}
                                    </div>
                                ))}
                            </div>
                        ...
                        <motion.blockquote
                            variants={itemVariants}
                            className="mt-4 md:mt-6 border-l-2 border-accent pl-4 md:pl-6 text-lg md:text-2xl font-space-grotesk text-accent italic font-medium leading-relaxed"
                        >
